...
import type { Aircraft } from "../shared/aircraft";
import type { DisplayConfig } from "../shared/config";
import { formatSpeed } from "../shared/format";
import { classifyGlyph, drawAircraftGlyph, GLYPH_SCALE } from "./aircraftGlyph";
import { computeSky, type Sky, type Tle } from "../shared/celestial";
import { ASTERISMS } from "../shared/stars";
...
export class Renderer {
  ...
  private runways: RunwaySegment[] = [];
  private airportLabels: AirportLabel[] = [];
  private fetchTle: () => Promise<Tle[]>;

  constructor(
    private canvas: HTMLCanvasElement,
    private getConfig: () => DisplayConfig,
    options?: { fetchTle?: () => Promise<Tle[]> },
  ) {
    this.fetchTle =
      options?.fetchTle ??
      (async () => {
        try {
          ...
          if (res.ok) return (await res.json()) as Tle[];
        } catch {
          /* noop */
        }
        return [];
  ...
  setAirports(runways: RunwaySegment[], airportLabels: AirportLabel[]): void {
    this.runways = runways;
    this.airportLabels = airportLabels;
  }
  ...
  private passesFilter(ac: Aircraft, cfg: DisplayConfig): boolean {
  ...
  private sampleAt(tr: Track, tt: number, cfg: DisplayConfig): GroundSample | null {
    const h = tr.history;
  ...
  private horizonM(cfg: DisplayConfig): number {
   ...
   */
  private withLabelRotation(cfg: DisplayConfig, ax: number, ay: number, draw: () => void): void {
  ...
  private screenHeading(tr: Track, tt: number, cfg: DisplayConfig, proj: ProjOpts): number {
  ...
  private drawOverlays(cfg: DisplayConfig, proj: ProjOpts): void {
    const ctx = this.ctx;
    const cx = this.w / 2;
    const cy = this.h / 2;
  ...
  private drawAirport(cfg: DisplayConfig, proj: ProjOpts): void {
    const ctx = this.ctx;
    const rwyRgb: [number, number, number] = [150, 180, 220];
    const labelByIcao = new Map(
      this.airportLabels.map((a) => [a.icao, a]),
    );

    const byAirport = new Map<string, RunwaySegment[]>();
    for (const seg of this.runways) {
      const list = byAirport.get(seg.icao) ?? [];
      list.push(seg);
      byAirport.set(seg.icao, list);
    }

    for (const [icao, segments] of byAirport) {
      let cx = 0;
      let cy = 0;
      let n = 0;
      for (const r of segments) {
        const a = this.toScreen([r.leLat, r.leLon], cfg, proj);
        const b = this.toScreen([r.heLat, r.heLon], cfg, proj);
        const widthM = r.widthMeters ?? 30;
        const wpx = Math.max(2.5, widthM * proj.pxPerM * 1.4);
      ...
      if (n) {
        cx /= n;
        cy /= n;
        const ap = labelByIcao.get(icao);
        const name = ap?.iata ?? ap?.icao ?? icao;
        ctx.save();
  ...
  private toScreen(ll: [number, number], cfg: DisplayConfig, proj: ProjOpts, altFt = 0): Point {
  ...
  private updateSky(cfg: DisplayConfig, now: number): void {
    const want =
  ...
  private projectSky(az: number, alt: number, cfg: DisplayConfig, proj: ProjOpts): Point {
    return projectSkyPoint(az, alt, proj, this.horizonM(cfg));
  }

  private drawSky(cfg: DisplayConfig, proj: ProjOpts): void {
    const ctx = this.ctx;
  ...
  private skyLabel(p: Point, text: string, cfg: DisplayConfig, alpha: number, color = "#AEB6C6"): void {
    const ctx = this.ctx;
  ...
  private drawDestArc(cfg: DisplayConfig, proj: ProjOpts, v: Visible): void {
    const ac = v.tr.ac;
  ...
  // --- comet trail ---
  private drawTrail(cfg: DisplayConfig, proj: ProjOpts, v: Visible, tt: number): void {
  ...
  private drawGlyph(cfg: DisplayConfig, v: Visible): void {
    const ctx = this.ctx;
  ...
  private drawLabels(cfg: DisplayConfig, nearestFirst: Visible[]): void {
    const limit =
  ...
  private labelLines(cfg: DisplayConfig, ac: Aircraft): { text: string; kind: "title" | "sub" }[] {
  ...
  private drawLabel(cfg: DisplayConfig, v: Visible, strength: number): void {
    const ctx = this.ctx;
  ...
  private drawDetailPanel(cfg: DisplayConfig, v: Visible): void {
    const ac = v.tr.ac;
    const x = 40;
    const y = this.h - 120;
  ...
  private drawDetailPanelText(cfg: DisplayConfig, v: Visible, ac: Aircraft, x: number, y: number): void {
    const ctx = this.ctx;
    ctx.save();
    ctx.shadowColor = "rgba(0,0,0,0.9)";
    ctx.shadowBlur = 10;
 ...
 */
function routePlausible(ac: Aircraft, cfg: DisplayConfig): boolean {
