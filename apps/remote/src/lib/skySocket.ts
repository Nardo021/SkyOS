...
export function connectSkySocket(
  ...
  const connect = () => {
    ...
    ws.onmessage = (ev) => {
      ...
      if (!parsed) return;
      if ("type" in parsed) {
        if (parsed.type === "snapshot") handlers.onSnapshot(parsed.snapshot);
        else if (parsed.type === "config") handlers.onConfig?.(parsed.config);
        else if (parsed.type === "tle") handlers.onTle?.(parsed.tles);
      } else {
